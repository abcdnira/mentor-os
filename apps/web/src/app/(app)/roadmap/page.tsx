...
import { Map, Loader2, RefreshCw, CheckCircle, Clock, Circle } from "lucide-react";
import { listRoadmap, generateRoadmap, updateRoadmapStatus, type RoadmapItem } from "@/lib/api";
    ...
    <div className="p-8 max-w-3xl mx-auto">
      <div className="flex items-center justify-between mb-6">
        <div>
          ...
          <p className="text-sm text-gray-500 mt-1">Your career growth path</p>
        </div>
        <button onClick={handleGenerate} disabled={generating}
          ...
          {generating ? <><Loader2 size={14} className="animate-spin" /> Generating...</>
            : <><RefreshCw size={14} /> {items.length > 0 ? "Regenerate" : "Generate Roadmap"}</>}
        </button>
      </div>
      ...
      {loading ? (
        ...
        <div className="text-center py-16">
          ...
          <p className="text-sm text-gray-400">
            No roadmap yet. Click &quot;Generate Roadmap&quot; to create one based on your goals.
          </p>
        </div>
      ) : (
              ...
              return (
                <div key={item.id} className="relative flex gap-4 pl-2">
                  ...
                  <button
                    ...
                    className={cn("relative z-10 w-7 h-7 rounded-full flex items-center justify-center shrink-0 transition hover:scale-110", cfg.bg)}
                  ...
                  <div className="flex-1 py-2">
                    <div className="flex items-center gap-2">
                      ...
                      {item.priority > 0 && item.priority <= 5 && (
                        <span className={cn("text-[10px] font-medium px-1.5 py-0.5 rounded",
                          item.priority <= 2 ? "bg-red-50 text-red-600" : "bg-gray-100 text-gray-500"
                        )}>
                    ...
                    {item.next_action && (
                      <p className="text-xs text-brand-600 mt-1">Next: {item.next_action}</p>
                    )}
                  </div>
                </div>
              );
